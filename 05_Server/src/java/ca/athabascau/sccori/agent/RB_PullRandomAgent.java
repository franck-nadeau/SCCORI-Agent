 ...
 * This agent does its best to minimize "overall" unit costs for random runs in
 * the SCCORI "pull" type of simulation.
 * It is not as good at spiked runs as the RulesBasedAgent.
 *
 * See PullSpikedAgent for a specialized RulesBased agents that is designed
 * to do better in a spiked run SCCORI simulation environment.
 *
 ...
 * @author MaRinus
 */
public class RB_PullRandomAgent implements TurnListener {
	private static final Log theLog = LogFactory
			.getLog(RB_PullRandomAgent.class);

	public final static String DISPLAY_NAME = "Rule Base Agent (Pull Random)";
	...
	private final int containerSize = 60; // Number of units container can
											// hold.
	 ...
	 */
	public RB_PullRandomAgent(SimulationInfo aSimulationInfo) {
	...
	public void executeTurn(TurnEvent e) {
		...
		boolean isRetailerBacklogged = false; // backlogged retailer; very bad
		...
		int theOrder = stockingLevel - theTurnInfo.getStockOnHand()
				- theTurnInfo.getLD1() - theTurnInfo.getLD2()
				- theTurnInfo.getLD3();
		...
		// In a backlogged situation, the stocks will eventually be large enough.
	...
	private static void testFourBots(final String aRoomName, final int aTime) {
		try {
			...
			theSimulation1.setAgent(new RB_PullRandomAgent(theSimulation1
					.getSimulationInfo()));

			// Bot 2
			Simulation theSimulation2 = new Simulation("BOT2_COMP667","qwerty");
			theSimulation2.joinMeeting(aRoomName);
			theSimulation2.setAgent(new RB_PullRandomAgent(theSimulation2
					.getSimulationInfo()));

			// Bot 3
			Simulation theSimulation3 = new Simulation("BOT3_COMP667","qwerty");
			theSimulation3.joinMeeting(aRoomName);
			theSimulation3.setAgent(new RB_PullRandomAgent(theSimulation3
					.getSimulationInfo()));

			// Bot 4
			final Simulation theSimulation4 = new Simulation("BOT4_COMP667","qwerty");
			theSimulation4.joinMeeting(aRoomName);
			theSimulation4.setAgent(new RB_PullRandomAgent(theSimulation4
					.getSimulationInfo()));
	...
	public static void main(String[] args) {
		...
		// testFourBots("BOT_ROOM7_PULL_RANDOM", 19);
